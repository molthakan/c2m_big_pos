...
if ($this->db->insert("whbig_importproduct_detail", $data2)){
		return true;
	}
...
$data2['user_id'] = $_SESSION['user_id'];
$data2['store_id'] = $_SESSION['store_id'];
if ($this->db->insert("whbig_importproduct_header", $data2)){
        return true;
    }
...
$querynum = $this->db->query('SELECT *, from_unixtime(importproduct_header_date,"%d-%m-%Y %H:%i:%s") as importproduct_header_date2
    FROM whbig_importproduct_header  
...
from_unixtime(wi.importproduct_detail_date,"%d-%m-%Y %H:%i:%s") as importproduct_detail_date
    FROM whbig_importproduct_detail as wi
...
$query = $this->db->query('DELETE FROM whbig_importproduct_detail  WHERE importproduct_header_code="'.$data['importproduct_header_code'].'" and  owner_id="'.$_SESSION['owner_id'].'"');

if($query){
$query2 = $this->db->query('DELETE FROM whbig_importproduct_header  WHERE importproduct_header_code="'.$data['importproduct_header_code'].'" and  owner_id="'.$_SESSION['owner_id'].'"');
}
...
$query = $this->db->query('UPDATE wh_product_list 
    SET product_whbig_num=product_whbig_num + '.$data['importproduct_detail_num'].',product_num_all=product_num_all + '.$data['importproduct_detail_num'].',
...
$query = $this->db->query('UPDATE wh_product_list 
    SET product_whbig_num=product_whbig_num - '.$data2['detailnum'].',
