...
$query = $this->db->query('SELECT 
    ...
    wl.product_whbig_num as product_whbig_num,
...
$query = $this->db->query('DELETE FROM whbig_exportproduct_datail  WHERE sale_runno="'.$data['sale_runno'].'" and  owner_id="'.$_SESSION['owner_id'].'"');

if($query){
$query2 = $this->db->query('DELETE FROM whbig_exportproduct_header  WHERE sale_runno="'.$data['sale_runno'].'" and  owner_id="'.$_SESSION['owner_id'].'"');
...
$query = $this->db->query('UPDATE wh_product_list 
    SET product_whbig_num=product_whbig_num + '.$data['product_sale_num'].'   
...
$query = $this->db->query('SELECT *, from_unixtime(adddate,"%d-%m-%Y %H:%i:%s") as adddate
    FROM whbig_exportproduct_datail
...
if ($this->db->insert("whbig_exportproduct_datail", $data)){
        return true;
    }
...
if ($this->db->insert("whbig_exportproduct_header", $data2)){
...
$query = $this->db->query('UPDATE wh_product_list 
    SET product_whbig_num=product_whbig_num - '.$data2['product_sale_num'].'   
...
$querynum = $this->db->query('SELECT *, from_unixtime(adddate,"%d-%m-%Y %H:%i:%s") as adddate
    FROM whbig_exportproduct_header  
